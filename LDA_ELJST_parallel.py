...
#         self.df_matrix = None
        ...
        self.docs_edges = None
        
    def store_data(self, train_matrix, train_sentiment, words, vocabulary, docs_edges, edge_dict, ids):
        matrix = np.copy(train_matrix)
...
#         self.df_matrix = df_matrix.copy()
    ...
    def _initialize(self, train_matrix, train_sentiment):
        matrix = np.copy(train_matrix)
...
#         V = self.nzws.shape[1]
    ...
    def run_sub(self, m):
        for i, w in enumerate(word_indices(self.count_matrix[m, :])):
            z = self.topics[(m,i)]
            ...
            p_z = self._conditional_distribution(m, w, self.edge_dict)
...
#         print("Done", m)
    ...
    def run(self, train_matrix, train_sentiment, edge_dict, maxiter=100, parallel=20):
        """
        Run the Gibbs sampler.
        """
        n_docs, vocab_size = train_matrix.shape

        self._initialize(train_matrix, train_sentiment)
        self.edge_dict = edge_dict
        
        self.count_matrix = self.get_count_matrix()

        for it in xrange(maxiter):
#             pool = pp.ProcessPool(parallel)
#             pool.map(self.run_sub, list(xrange(n_docs)))

            pool = Pool(processes=parallel)
            async_results = [ pool.apply_async(self, (i,)) for i in range(n_docs) ]
            pool.close()
